...
// Queries come from the taxonomy, never from the raw prompt (spec 4.2.4).
// Results are cached 24h per domain; a failed lookup falls back to a stale
// cache entry rather than erroring the hook.
export async function discover(domain: DomainDef, opts: DiscoverOpts = {}): Promise<Candidate[]> {
  const now = opts.now ?? new Date();
  const cache = readCache();
  const hit = cache.discovery[domain.id];
  if (hit && now.getTime() - Date.parse(hit.ts) < CACHE_TTL_MS) return hit.candidates;

  const runner = opts.runner ?? defaultRunner;
  try {
    const { stdout } = await runner([...skillsCmd(), "find", domain.query], opts.timeoutMs ?? 10_000);
    const candidates = parseFindOutput(stdout);
    cache.discovery[domain.id] = { ts: now.toISOString(), candidates };
    writeCache(cache);
    return candidates;
  } catch {
