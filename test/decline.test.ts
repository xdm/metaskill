...
import { freshHome, logRows, rec, runCli, writeIndex } from "./helpers/cli.js";
...
describe("decline command", () => {
  it("records the package with the phrase, logs one row, and asks nothing of the network", async () => {
    ...
    // No index in this home, so nothing is known about the package beyond
    // its name: the row carries it in `domains` and invents no numbers.
  ...
  it("logs the package's install count and scan verdict when the index knows it", async () => {
    const home = freshHome("record-indexed");
    fs.mkdirSync(path.join(home, ".metaskill"), { recursive: true });
    fs.writeFileSync(
      path.join(home, ".metaskill", "index.json"),
      JSON.stringify({ schemaVersion: 1, builtAt: "t", skillCount: 1, repoCount: 1, skills: [
        rec({ name: "proptech-advisor", source: "borghei/claude-skills", pkg: "borghei/claude-skills@proptech-advisor",
              description: "Proptech.", installs: 4757, scan: "clean" }),
      ] }),
    );
    await runCli(["decline", "borghei/claude-skills@proptech-advisor"], { home });
    expect(logRows(home)[0].discovered).toEqual([
      { pkg: "borghei/claude-skills@proptech-advisor", installs: 4757, publisher: "borghei", decision: "declined", scan: "clean" },
    ]);
  });

  it("fails in one line, logging nothing, when the record cannot be written", async () => {
    // The model runs this from a printed line; a stack trace teaches it the
    // command is broken. And nothing is logged for a no that was not
    // recorded: a log row for a decline that never landed would count as an
    // answered question in `log --stats`.
    const home = freshHome("unwritable");
    fs.mkdirSync(path.join(home, ".metaskill", "declined.json"), { recursive: true }); // a directory in the file's place
    const r = await runCli(["decline", "o/r@skill", "--matched", "x"], { home });
    expect(r.code).toBe(1);
    expect(r.stderr.split("\n").filter(Boolean)).toHaveLength(1);
    expect(r.stderr).toMatch(/^decline failed: /);
    expect(r.stderr).not.toMatch(/    at /);
    expect(logRows(home)).toEqual([]);
  });
...
describe("find hides a declined package", () => {
  it("the question moves to the next row and the hidden one is named under the list", async () => {
    ...
    // The log carries the hidden row too, marked as declined, after the rows
    // that were printed — so `metaskill log` never reads "found one" for a
    // lookup that found two and hid one.
    const last = logRows(home).at(-1);
    expect(last.discovered).toEqual([
      expect.objectContaining({ pkg: "101-skills/superpowers@linkedin-content", decision: "ask" }),
      { pkg: "borghei/claude-skills@proptech-advisor", installs: 300, publisher: "borghei", decision: "declined", scan: "clean" },
    ]);
  });
  ...
  it("with every match declined, find says no skills found", async () => {
    ...
    // Both hidden rows reach the log as declined — not `discovered: []`,
    // which reads as "the index has nothing for this query".
    expect(logRows(home).at(-1).discovered.map((d: { pkg: string; decision: string }) => [d.pkg, d.decision])).toEqual([
      ["borghei/claude-skills@proptech-advisor", "declined"],
      ["101-skills/superpowers@linkedin-content", "declined"],
    ]);
  });
});
